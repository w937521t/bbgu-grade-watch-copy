...
test('curl单次传输通过标准输入隐藏Token并解析响应', async () => {
  let invocation;
  const result = await requestWithCurl('https://zhjw.bbgu.edu.cn/api/test', {
    method: 'POST',
    headers: {
    ...
    randomUUIDFn: () => 'test-marker',
    executeCurlFn: async (nextInvocation) => {
      invocation = nextInvocation;
      return {
        exitCode: 0,
        stdout: '{"status":"success"}__BBGU_CURL_test-marker__200',
        stderr: '',
  ...
  assert.doesNotMatch(invocation.args.join(' '), /secret-access-token|secret-refresh-token/);
  assert.match(invocation.stdinText, /secret-access-token/);
  assert.match(invocation.stdinText, /secret-refresh-token/);
...
test('curl连接失败保留阶段并进入安全节点切换分类', async () => {
  await assert.rejects(requestWithCurl('https://zhjw.bbgu.edu.cn/api/test', {
    proxyServer: 'http://127.0.0.1:7890',
  }, {
    randomUUIDFn: () => 'curl-connect-failed',
    executeCurlFn: async () => ({
      exitCode: 7,
      stdout: '',
      stderr: 'curl: (7) Failed to connect to proxy',
      headersText: '',
    }),
    rmFn: async () => undefined,
  }), (error) => error
    && error.stage === 'proxy-tcp'
    && error.code === 'BBGU_CURL_7'
    && isNetworkTransportError(error)
    && isSafeApiFailoverError(error));
});
...
test('Watch双节点失败只结束本次任务且下一次Watch继续执行', async () => {
  ...
  const networkStatePath = path.join(tempDir, 'network.json');
  const config = { networkStatePath };
  const networkError = Object.assign(new Error('双节点均不可用'), {
    code: 'BBGU_PROXY_FAILOVER_EXHAUSTED',
  });
  let runCount = 0;
  try {
    ...
    await assert.rejects(fs.access(networkStatePath), { code: 'ENOENT' });
...
test('旧skipNextWatch状态被清理且当前Watch继续执行', async () => {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'bbgu-old-watch-network-'));
  const networkStatePath = path.join(tempDir, 'network.json');
  const config = { networkStatePath };
  let runCount = 0;
  try {
    await fs.writeFile(networkStatePath, JSON.stringify({ skipNextWatch: true, failedAt: 1000 }));
    const result = await run(config, {
      runCoreFn: async () => {
        runCount += 1;
        return { status: 'ok' };
...
test('请求已发出后的网络失败不跨IP重试且不建立跨任务退避', async () => {
  const calls = [];
  const config = {
    pushplusToken: 'push-token',
    term: '2026春',
...
test('代理模式Refresh只使用一次curl且不运行旧传输', async () => {
  const calls = [];
  const result = await requestRefreshedAuthState(
    {
      homeUrl: 'https://zhjw.bbgu.edu.cn/workspace/home',
  ...
  assert.equal(calls[0].url, 'https://zhjw.bbgu.edu.cn/authserver/oauth/token');
  assert.equal(calls[0].options.proxyServer, 'http://127.0.0.1:7890');
  assert.match(calls[0].options.body, /grant_type=refresh_token/);
  assert.equal(result.accessToken, 'access.new.token');
  assert.equal(result.refreshToken, 'refresh.new.token');
});

test('Refresh仅在curl确认未发送HTTP的TLS阶段允许切换一个节点', async () => {
  let curlCalls = 0;
  const result = await requestRefreshedAuthState({
    homeUrl: 'https://zhjw.bbgu.edu.cn/workspace/home',
    proxyServer: 'http://127.0.0.1:7890',
    ...
    withProxyFailoverFn: async (_config, operation, options) => {
      assert.equal(options.shouldFailoverFn, isSafeApiFailoverError);
      try {
        await operation();
      } catch (error) {
        assert.equal(options.shouldFailoverFn(error), true);
      }
      return operation();
    },
    curlRequestFn: async () => {
      curlCalls += 1;
      if (curlCalls === 1) {
        throw Object.assign(new Error('target-tls: handshake failed'), {
          stage: 'target-tls',
          code: 'BBGU_CURL_35',
        });
      }
  ...
  await assert.rejects(requestRefreshedAuthState({
    ...
    withProxyFailoverFn: async (_config, operation, options) => {
      try {
        ...
        assert.equal(options.shouldFailoverFn(error), false);
        throw error;
      }
    },
    curlRequestFn: async () => {
